...
Keywords
none*/
import { basic } from "./ProtectedRoute1";
export const getText = (text) => {
  const presentWords = [];
  const words = text.split(' ');
  words.forEach(word => {
    const trimmedWord = word.trim().toLowerCase();;
      if (basic.includes(trimmedWord)) {
          presentWords.push(trimmedWord);
      }
  });

  let concatenatedString = presentWords.join(' ');

  if (!/bird[s]?/i.test(concatenatedString)) {
    console.log("red",concatenatedString)
      return -1;
  }
